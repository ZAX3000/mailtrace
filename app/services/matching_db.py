...
if TYPE_CHECKING:
    # Only for typing; avoids import cycles at runtime
    from app.models import Run


def _fetch_mail_df() -> pd.DataFrame:
    """
    Only the columns the matcher needs from staging.mail.
    """
    sql = """
        SELECT
          COALESCE(id, '')              AS id,
...
def _fetch_crm_df() -> pd.DataFrame:
    """
    Only the columns the matcher needs from staging.crm.
    """
    sql = """
        SELECT
...
def _parse_mmddyy_to_date(s: Any) -> Optional[date]:
    ...
    Use the shared parse_date_any helper (handles many formats).
    """
...
def _extract_last_mail_date(mail_dates_in_window: str) -> Optional[date]:
    ...
    Return the last (most recent) non-empty date if available.
    """
    ...
    for p in reversed(parts):  # assume ascending -> check from rightmost
...
def _df_to_match_rows(df: pd.DataFrame, *, run_id: UUID, user_id: Optional[UUID]) -> List[Dict[str, Any]]:
    """
    Transform the matcher DataFrame into a list[dict] suitable for bulk_insert_matches.
    """
...
def run_matching_from_staging(*, user_id: Optional[UUID]) -> Dict[str, Any]:
    """
    Pull data from staging tables, run the pandas matcher, save results to matches,
    update the run, and return a compact dashboard payload.
    """
    # mypy: treat Flask-SQLAlchemy's scoped_session as a plain Session at call sites
    session: SASession = cast(SASession, db.session)

    # 1) Create a run record
    with session.begin():
        run: "Run" = create_run(session, user_id=user_id)

    # 2) Read staging sources (outside txn, no locks)
    mail_df = _fetch_mail_df()
    crm_df = _fetch_crm_df()

    # 3) Run pandas matcher
    df = run_matching(mail_df, crm_df)

    # 4) Build match rows and insert
    rows: List[Dict[str, Any]] = _df_to_match_rows(df, run_id=run.id, user_id=user_id)

    with session.begin():
        n_inserted: int = bulk_insert_matches(session, rows)
        # Keep your original finalize_run call shape (run object + kwargs)
        finalize_run(
            session,
            run,
    ...
    # 7) Summary table rows (shape the UI expects)
