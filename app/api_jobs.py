...
def _set_job_progress(job_id, percent=None, phase=None, status=None, result=None, error=None):
    with JOBS_LOCK:
        st = JOBS.get(job_id, {"status": "running", "percent": 0, "phase": "loading"})
        if percent is not None: st["percent"] = int(percent)
        if phase is not None: st["phase"] = str(phase)
        if status is not None: st["status"] = status
        if result is not None: st["result"] = result
...
def match_start():
    ...
    fuzzy = True
    mode = (request.form.get('mode') or 'loose').lower()
    ...
    def worker():
        ...
        def progress_cb(pct, phase):
            _set_job_progress(job_id, percent=pct, phase=phase)
        try:
            try:
                mail_df = pd.read_csv(BytesIO(mail_bytes))
            except Exception:
                mail_df = pd.read_csv(BytesIO(mail_bytes), encoding='latin-1')
            try:
                crm_df  = pd.read_csv(BytesIO(crm_bytes))
            except Exception:
                crm_df  = pd.read_csv(BytesIO(crm_bytes), encoding='latin-1')

            _set_job_progress(job_id, percent=2, phase='canonicalize')
            df = run_matching(mail_df, crm_df, fuzzy=fuzzy, mode=mode, progress_cb=progress_cb)

            total_mail = len(mail_df)
            uniqmail = (mail_df[['address1','address2','city','state','postal_code']].drop_duplicates().shape[0]
                        if set(['address1','address2','city','state','postal_code']).issubset(set(mail_df.columns)) else len(mail_df))
            total_jobs = len(crm_df)
            matches = len(df)
            match_rate = (matches/total_mail)*100 if total_mail else 0.0
            try:
                match_rev = float(pd.to_numeric(df.get('job_value', 0), errors='coerce').fillna(0).sum())
            except Exception:
                match_rev = 0.0
            kpis = {'mail': total_mail, 'uniqmail': uniqmail, 'crm': total_jobs,
                    'matches': matches, 'match_rate': round(match_rate,2), 'match_revenue': round(match_rev,2)}

            # YoY graph (months aligned, prev year dashed)
            dt = pd.to_datetime(df.get('crm_job_date'), errors='coerce')
            df['_y'] = dt.dt.year
            df['_m'] = dt.dt.strftime('%m-%Y')
            years = df['_y'].dropna().astype(int)
            if not years.empty:
                cur_year = int(years.max()); prev_year = cur_year - 1
            ...
            try:
                top_cities = df['mail_city'].value_counts().head(10).reset_index().values.tolist()
            except Exception:
                top_cities = []
            try:
                top_zips = df['mail_zip'].astype(str).str[:5].value_counts().head(10).reset_index().values.tolist()
            except Exception:
                top_zips = []

            result = {'kpis': kpis, 'graph': graph, 'summary': summary, 'top_cities': top_cities, 'top_zips': top_zips}
...
def match_progress():
    ...
    return jsonify({'status': st.get('status','running'), 'percent': st.get('percent',0), 'phase': st.get('phase','')})
...
# ---- Added JSON endpoints for navbar/history and cumulative data ----
...
def _row_to_dict(m):
    return dict(
        ...
        crm_city=m.crm_city or "", crm_state=m.crm_state or "", crm_zip=m.crm_zip or ""
    )
...
def _dedup_records(rows):
    ...
    out = []
    for r in rows:
        key = (r.matched_mail_full_address or "", r.crm_job_date.strftime("%Y-%m-%d") if r.crm_job_date else "")
        if key in seen:
            continue
        seen.add(key)
        out.append(r)
...
def list_runs():
    ...
    try:
        ...
        # Normalize UUID for SQLite-safe querying
        uid_val = None
        try:
...
def _build_payload(rows, mail_total=0):
    ...
    top_cities = [{"city": c[0], "state": c[1], "count": n} for c,n in sorted(city_counts.items(), key=lambda x: -x[1])[:20]]
...
@api.route("/run_result", methods=["GET"])
def run_result():
    uid = session.get("user_id")
    run_id = request.args.get("run_id")
    if not uid or not run_id:
        return jsonify(_build_payload([]))
    rows = db.session.query(Match).filter(Match.user_id==uid, Match.run_id==run_id).all()
    rows = _dedup_records(rows)
    mail_total = sum([(r.mail_count_in_window or 0) for r in rows])
    return jsonify(_build_payload(rows, mail_total=mail_total))