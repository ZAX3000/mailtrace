...
def _fmt_money(n: float | int | str) -> str:
    try:
        return f"${float(n):,.2f}"
    except (TypeError, ValueError):
        return f"${_to_num(n):,.2f}"
...
def _parse_date_any(s: str | object) -> Optional[date]:
    ...
    if pd.isna(d):
        return None
    # pandas can produce Timestamp; .date() exists on both Timestamp and datetime
    return d.date()  # type: ignore[no-any-return]


def _parse_many_dates(csvish: str) -> List[str]:
    if not isinstance(csvish, str) or not csvish.strip():
        return []
...
def _fmt_crm_date(s: object) -> str:
...
def _split_mail_address(full: str) -> Tuple[str, str]:
    s = _norm_str(full)
    if not s:
        return "", ""
    # Try "..., CITY, ST ZIP"
    if "," in s:
        ...
        for i in range(len(parts) - 1, -1, -1):
            ...
            if re.search(r"\b[A-Z]{2}\b", tail) and re.search(r"\b\d{5}(?:-\d{4})?\b", tail):
                left = ", ".join(parts[:i])
                return left, tail
    # Try "... ST 12345" with no city comma
...
def finalize_summary_for_export_v17(df: pd.DataFrame | None) -> pd.DataFrame:
    ...
    if crm_a2 in d.columns:
        ...
        mask = extra.ne("")
        d.loc[mask, "crm_street"] = d.loc[mask, "crm_street"] + ", " + extra[mask]

    d["crm_city_state_zip"] = d.apply(
        lambda r: _city_state_zip(r.get(crm_city, ""), r.get(crm_state, ""), str(r.get(crm_zip, ""))), axis=1
    )
    ...
    if mail_dates in d.columns:
        d["mail_dates"] = (
            d[mail_dates].fillna("").astype(str).map(_parse_many_dates).map(lambda L: ", ".join(L) if L else "")
        )
...
def _calc_kpis(d: pd.DataFrame, mail_count_total: int) -> Dict[str, float | int]:
...
def _top_counts(
    d: pd.DataFrame, city_col: str, state_col: str, zip_col: str, limit: int = 5
) -> Tuple[List[Tuple[str, int]], List[Tuple[str, int]]]:
    if d is None or d.empty:
        return [], []
    tmp = pd.DataFrame(
        {"city_state": d["crm_city_state_zip"].fillna(""), "zip": d[zip_col] if zip_col in d.columns else ""}
    )

    def _just_city_state(s: str) -> str:
        if not s:
            return ""
        m = re.match(r"^(.*?\b[A-Za-z\.\s]+,\s*[A-Z]{2})\b", s)
        if m:
...
def _month_series(d: pd.DataFrame) -> pd.Series:
    if d is None or d.empty:
    ...
    if ser.empty:
        return pd.Series(dtype=int)
    months = ser.map(lambda dt: dt.replace(day=1))  # type: ignore[union-attr]
...
def render_full_dashboard_v17(summary_df: pd.DataFrame, mail_count_total: int) -> str:
    ...
    month_counts = _month_series(d)
    month_png = _plot_month_chart(month_counts)

    rows_html: List[str] = []
    view_limit = min(200, len(d))
    if view_limit:

        def _latest_mail_date(csvish: str) -> Optional[date]:
        ...
        for _, r in d.iterrows():
            ...
            crm_date_s = _norm_str(r.get("crm_job_date", ""))
            ...
            <tr>
              ...
              <td class="td-date">{crm_date_s or ""}</td>
    ...
    cities_html = _list_block(top_cities)
    zips_html = _list_block(top_zips)
    chart_html = (
        f'<img class="month-chart" alt="Matched Jobs by Month" src="data:image/png;base64,{month_png}"/>'
        if month_png
        else '<div class="muted">No monthly data</div>'
    )
