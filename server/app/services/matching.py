...
try:
    MATCH_MIN_SCORE: float = float((os.getenv("MATCH_MIN_SCORE") or "").strip() or "0")
...
def run_matching(mail_df: pd.DataFrame, crm_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, c in crm_df.iterrows():
        ...
        prior_dates: List[Optional[date]] = []
        for _, m in cand.iterrows():
            d = m.get("_date")
    ...
    try:
        ...
        # If the environment doesn't allow writing (e.g., read-only CI),
        # skip silently—this artifact is optional for the UI.
        pass
